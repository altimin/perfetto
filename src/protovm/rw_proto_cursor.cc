...
RwProtoCursor RwProtoCursor::RepeatedFieldIterator::operator*() {
...
StatusOr<void> RwProtoCursor::EnterRepeatedFieldByIndex(uint32_t field_id,
                                                        uint32_t index) {
  PERFETTO_DCHECK(node_);
...
StatusOr<UniquePtr<Node>> RwProtoCursor::CreateNodeFromField(
...
StatusOr<uint64_t> RwProtoCursor::ReadScalarField(Node& node,
                                                  uint32_t field_id) {
